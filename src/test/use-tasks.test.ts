...
describe('useTasks', () => {
  ...
  it('loads tasks from localStorage on mount', () => {
    const stored = [
      { id: 1, text: 'Existing task', done: false, priority: 'none' },
    ]
  ...
  it('clearCompleted removes only completed tasks', () => {
    const { result } = renderHook(() => useTasks())
    act(() => {
      result.current.addTask('Keep me')
    })
    act(() => {
      result.current.addTask('Remove me')
    })
  ...
  it('cyclePriority cycles through none → low → medium → high → none', () => {
    const { result } = renderHook(() => useTasks())
    act(() => {
      result.current.addTask('Priority task')
    })
    const id = result.current.tasks[0].id
    expect(result.current.tasks[0].priority).toBe('none')
    act(() => {
      result.current.cyclePriority(id)
    })
  ...
  it('persists tasks to localStorage after mutations', () => {
    ...
    const stored = JSON.parse(localStorage.getItem('tasks') ?? '[]') as Array<{
      text: string
    }>
